...
if (isset($_POST['edit'],
    ...
    if (!count($errors)) {
        ...
        $currentLogin = mysqli_fetch_assoc($currentUser);
        if ($_POST['login'] != $currentLogin['login']) {
            $res = query("
        ...
        $currentEmail = mysqli_fetch_assoc($currentUser);
        if ($_POST['email'] != $currentEmail['email']) {
